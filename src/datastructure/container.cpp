...
namespace cx::workspace
{
    ...
    ContainerTree::ContainerTree(std::string container_tag, geom::Geometry geometry) noexcept
        : tag(std::move(container_tag)), client{}, left(nullptr), right(nullptr), parent(nullptr), policy(Layout::Vertical),
          split_ratio(0.5), geometry(geometry), height(0)
    {
    ...
    ContainerTree::ContainerTree(std::string container_tag, geom::Geometry geometry, Layout layout) noexcept
        : tag(std::move(container_tag)), client{}, left(nullptr), right(nullptr), parent(this), policy(layout), split_ratio(0.5f),
    ...
    {
        if (is_window()) {
            DBGLOG("Destroying Window Container. Client id: {} - Frame id: {}. Window tag: {} on workspace {}", client->client_id,
                   client->frame_id, client->m_tag.m_tag, client->m_tag.m_ws_id);
        }
    }
