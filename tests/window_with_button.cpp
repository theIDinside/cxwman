...
static xcb_gc_t gc_button_get(xcb_connection_t* c, xcb_screen_t* screen, xcb_window_t window, int fg_color = 0x000000)
{
...
static void mybutton(xcb_connection_t* c, xcb_screen_t* screen, xcb_window_t window, int16_t x1, int16_t y1, int width, int height, int color = 0x000000) {
    auto gc = gc_button_get(c, screen, window, color);

    auto mask = XCB_CW_BACK_PIXEL;
    int values[1]{color};

    auto cookie = xcb_change_window_attributes_checked(c, window, mask, values);
    if(auto error = xcb_request_check(c, cookie); error) {
        fprintf(stderr, "ERROR: can't free gc : %d\n", error->error_code);
        xcb_disconnect(c);
        exit(-1);
    }
    xcb_clear_area(c, 1, window, x1, y1, width, height);
...
/*
    ...
    r->width = width;
    r->height = height;
    auto cookie = xcb_poly_fill_rectangle_checked(c, window, gc, 1, r);
    if(auto err = xcb_request_check(c, cookie); err) {
        fprintf(stderr, "ERROR: can't free gc : %d\n", err->error_code);
    }
    auto cookie_gc = xcb_free_gc(c, gc);
    if(auto error = xcb_request_check(c, cookie_gc); error) {
        fprintf(stderr, "ERROR: can't free gc : %d\n", error->error_code);
        xcb_disconnect(c);
        exit(-1);
    }
...
int main()
{
    ...
    cookie_window = xcb_create_window_checked(c, screen->root_depth, window, screen->root, 20, 200, WIDTH, HEIGHT, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT,
    ...
    auto btn_cookie = xcb_create_window_checked(c, screen->root_depth, btn_id, window, bx, by, bw, bh, 1, XCB_WINDOW_CLASS_INPUT_OUTPUT, screen->root_visual,
                                        mask, values);
    auto btn_map_cookie = xcb_map_window_checked(c, btn_id);
    if(auto e = xcb_request_check(c, btn_cookie)) {
        printf("ERROR: can't create window : %d\n", error->error_code);
    }
    if(auto e = xcb_request_check(c, btn_map_cookie)) {
        printf("ERROR: can't create window : %d\n", error->error_code);
    }

    mybutton(c, screen, btn_id, bx, by, bw, bh, 0xff00ff);
    xcb_flush(c);
    auto clicked = false;
    while(1) {
        ...
        if(e) {
            ...
            case XCB_EXPOSE: {
                ...
                mybutton(c, screen, btn_id, 0, 0, bw, bh, (clicked ? 0xff0000 : 0x00ff00));
                break;
            }
            case XCB_BUTTON_PRESS: {
                ...
                clicked = !clicked;
                mybutton(c, screen, btn_id, 0, 0, bw, bh, (clicked ? 0xff0000 : 0x0000ff));
                if(ev->event == btn_id)
                    printf("You clicked button!!!\n");
                length = strlen("click here to change cursor");

                if((ev->event_x >= (WIDTH - 7 * length) / 2) && (ev->event_x <= ((WIDTH - 7 * length) / 2 + 7 * length + 6)) &&
                   (ev->event_y >= (HEIGHT - 16) / 2 - 19) && (ev->event_y <= ((HEIGHT - 16) / 2)))
                    is_hand = 1 - is_hand;
